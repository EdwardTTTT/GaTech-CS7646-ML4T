...
def testPolicy(symbol="AAPL", sd=dt.datetime(2010, 1, 1), ed=dt.datetime(2011, 12, 31), sv=100000):
    """
    Implement Theoretically Optimal Strategy.
    ...
    for i in range(prices.shape[0] - 1):
        if prices.iloc[i + 1][symbol] > prices.iloc[i][symbol]:  # Compare individual elements
            df_trades.iloc[i] = 1000.0  # Buy 1000 shares
        elif prices.iloc[i + 1][symbol] < prices.iloc[i][symbol]:  # Compare individual elements
            df_trades.iloc[i] = -1000.0  # Sell 1000 shares
        else:
            df_trades.iloc[i] = 0.0  # Do nothing
    
    # Limit trades to ensure only -1000, 0, or 1000 holdings
    for i in range(1, df_trades.shape[0]):
        df_trades.iloc[i] += df_trades.iloc[i - 1]
        if df_trades.iloc[i][symbol] > 1000.0:
            df_trades.iloc[i] = 1000.0
        elif df_trades.iloc[i][symbol] < -1000.0:
            df_trades.iloc[i] = -1000.0
